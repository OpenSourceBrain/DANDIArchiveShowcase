...
docker run -v /var/run/docker.sock:/var/run/docker.sock -v $PWD:/testing -v $PWD/Summaries:/Summaries -v tmpf:/tmp --memory="8g" -e PYTHONBUFFERED=1 -d --name=da dandi_docker tail -f /dev/null
